...
type QRLRPCTestSuite struct {
	suite.Suite
	rpc *QRLRPC
}

func (s *QRLRPCTestSuite) registerResponse(result string, callback func([]byte)) {
	httpmock.Reset()
...
func (s *QRLRPCTestSuite) registerResponseError(err error) {
	httpmock.Reset()
	httpmock.RegisterResponder("POST", s.rpc.url, func(request *http.Request) (*http.Response, error) {
		return nil, err
	})
...
func (s *QRLRPCTestSuite) getBody(request *http.Request) []byte {
...
func (s *QRLRPCTestSuite) methodEqual(body []byte, expected string) {
	response := struct {
...
func (s *QRLRPCTestSuite) paramsEqual(body []byte, expected string) {
	response := struct {
...
func (s *QRLRPCTestSuite) SetupSuite() {
	s.rpc = NewQRLRPC("http://127.0.0.1:8545", WithHttpClient(http.DefaultClient), WithLogger(nil), WithDebug(false))

	httpmock.Activate()
}

func (s *QRLRPCTestSuite) TearDownSuite() {
	httpmock.Deactivate()
}

func (s *QRLRPCTestSuite) TearDownTest() {
	httpmock.Reset()
}

func (s *QRLRPCTestSuite) TestURL() {
	s.Require().Equal(s.rpc.url, s.rpc.URL())
}

func (s *QRLRPCTestSuite) TestWeb3ClientVersion() {
...
func (s *QRLRPCTestSuite) TestCall() {
	// Test http error
	httpmock.RegisterResponder("POST", s.rpc.url, func(request *http.Request) (*http.Response, error) {
	...
	_, err = s.rpc.Call("test")
	s.Require().NotNil(err)
	qrlError, ok := err.(QRLError)
	s.Require().True(ok)
	s.Require().Equal(21, qrlError.Code)
	s.Require().Equal("eee", qrlError.Message)
}

func (s *QRLRPCTestSuite) Test_call() {
	// Test http error
	httpmock.RegisterResponder("POST", s.rpc.url, func(request *http.Request) (*http.Response, error) {
...
func (s *QRLRPCTestSuite) TestWeb3Sha3() {
...
func (s *QRLRPCTestSuite) TestNetVersion() {
...
func (s *QRLRPCTestSuite) TestNetListening() {
...
func (s *QRLRPCTestSuite) TestNetPeerCount() {
	// Test error
...
func (s *QRLRPCTestSuite) TestQRLProtocolVersion() {
	s.registerResponse(`"54"`, func(body []byte) {
		s.methodEqual(body, "qrl_protocolVersion")
		s.paramsEqual(body, "null")
	})

	protocolVersion, err := s.rpc.QRLProtocolVersion()
	s.Require().Nil(err)
	s.Require().Equal("54", protocolVersion)
}

func (s *QRLRPCTestSuite) TestQRLSyncing() {
	s.registerResponseError(errors.New("Error"))
	syncing, err := s.rpc.QRLSyncing()
	s.Require().NotNil(err)
	...
	s.registerResponse(`false`, func(body []byte) {
		s.methodEqual(body, "qrl_syncing")
	})
	syncing, err = s.rpc.QRLSyncing()
...
func (s *QRLRPCTestSuite) TestQRLCoinbase() {
	s.registerResponse(`"0x407d73d8a49eeb85d32cf465507dd71d507100c1"`, func(body []byte) {
		s.methodEqual(body, "qrl_coinbase")
		s.paramsEqual(body, "null")
	})

	address, err := s.rpc.QRLCoinbase()
	s.Require().Nil(err)
	s.Require().Equal("0x407d73d8a49eeb85d32cf465507dd71d507100c1", address)
}
func (s *QRLRPCTestSuite) TestQRLMining() {
	s.registerResponse(`true`, func(body []byte) {
		s.methodEqual(body, "qrl_mining")
		s.paramsEqual(body, "null")
	})

	mining, err := s.rpc.QRLMining()
	s.Require().Nil(err)
...
func (s *QRLRPCTestSuite) TestQRLAccounts() {
	s.registerResponse(`["0x407d73d8a49eeb85d32cf465507dd71d507100c1"]`, func(body []byte) {
		s.methodEqual(body, "qrl_accounts")
		s.paramsEqual(body, "null")
	})

	accounts, err := s.rpc.QRLAccounts()
	s.Require().Nil(err)
	s.Require().Equal([]string{"0x407d73d8a49eeb85d32cf465507dd71d507100c1"}, accounts)
}

func (s *QRLRPCTestSuite) TestQRLBlockNumber() {
	s.registerResponseError(errors.New("Error"))
	blockBumber, err := s.rpc.QRLBlockNumber()
	s.Require().NotNil(err)

	s.registerResponse(`"0x37eb38"`, func(body []byte) {
		s.methodEqual(body, "qrl_blockNumber")
		s.paramsEqual(body, "null")
	})
...
func (s *QRLRPCTestSuite) TestQRLGetBalance() {
	...
	s.registerResponse(`"0x486d06b0d08d05909c4"`, func(body []byte) {
		s.methodEqual(body, "qrl_getBalance")
...
func (s *QRLRPCTestSuite) TestQRLGetStorageAt() {
	...
	s.registerResponse(`"0x00000000000000000000000000000000000000000000000000000000000004d2"`, func(body []byte) {
		s.methodEqual(body, "qrl_getStorageAt")
		s.paramsEqual(body, fmt.Sprintf(`["%s", "0x21", "pending"]`, data))
	})

	result, err := s.rpc.QRLGetStorageAt(data, position, tag)
	s.Require().Nil(err)
	s.Require().Equal("0x00000000000000000000000000000000000000000000000000000000000004d2", result)
}

func (s *QRLRPCTestSuite) TestQRLGetTransactionCount() {
	address := "0x407d73d8a49eeb85d32cf465507dd71d507100c1"
	s.registerResponseError(errors.New("Error"))
	count, err := s.rpc.QRLGetTransactionCount(address, "latest")
	s.Require().NotNil(err)

	s.registerResponse(`"0x10"`, func(body []byte) {
		s.methodEqual(body, "qrl_getTransactionCount")
		s.paramsEqual(body, fmt.Sprintf(`["%s", "latest"]`, address))
	})
...
func (s *QRLRPCTestSuite) TestQRLGetBlockTransactionCountByHash() {
	hash := "0xb903239f8543d04b5dc1ba6579132b143087c68db1b2168786408fcbce568238"
	s.registerResponseError(errors.New("Error"))
	count, err := s.rpc.QRLGetBlockTransactionCountByHash(hash)
	s.Require().NotNil(err)

	s.registerResponse(`"0xb"`, func(body []byte) {
		s.methodEqual(body, "qrl_getBlockTransactionCountByHash")
...
func (s *QRLRPCTestSuite) TestQRLGetBlockTransactionCountByNumber() {
	number := 2384732
	s.registerResponseError(errors.New("Error"))
	count, err := s.rpc.QRLGetBlockTransactionCountByNumber(number)
	s.Require().NotNil(err)

	s.registerResponse(`"0xe8"`, func(body []byte) {
		s.methodEqual(body, "qrl_getBlockTransactionCountByNumber")
...
func (s *QRLRPCTestSuite) TestSendTransaction() {
	...
	result := "0xea1115eb5"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "qrl_sendTransaction")
		s.paramsEqual(body, `[{
			"from": "0x3cc1a3c082944b9dba70e490e481dd56",
			"to": "0x1bf21cb1dc384d019a885a06973f7308",
			"gas": "0x6144",
	...
	txid, err := s.rpc.QRLSendTransaction(t)
	s.Require().Nil(err)
...
func (s *QRLRPCTestSuite) TestQRLSendRawTransaction() {
	data := "0xd46e8dd67c5d32be8d46e8dd67c5d32be8058bb8eb970870f072445675058bb8eb970870f072445675"
	result := "0xe670ec64341771606e55d6b4ca35a1a6b75ee3d5145a99d05921026d1527331"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "qrl_sendRawTransaction")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, data))
	})

	txid, err := s.rpc.QRLSendRawTransaction(data)
	s.Require().Nil(err)
	s.Require().Equal(result, txid)
}

func (s *QRLRPCTestSuite) TestQRLGetCompilers() {
	s.registerResponse(`["solidity", "some comp"]`, func(body []byte) {
		s.methodEqual(body, "qrl_getCompilers")
		s.paramsEqual(body, "null")
	})
...
func (s *QRLRPCTestSuite) TestGetBlock() {
	s.registerResponseError(errors.New("Error"))
	block, err := s.rpc.getBlock("qrl_getBlockByHash", true)
	s.Require().NotNil(err)
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "qrl_getBlockByHash")
	})
	...
	block, err = s.rpc.getBlock("qrl_getBlockByHash", false)
	s.Require().Nil(err)
	s.Require().NotNil(block)
	s.Require().Equal(hash, block.Hash)
...
func (s *QRLRPCTestSuite) TestQRLGetBlockByHash() {
	// Test with transactions
	hash := "0x111"
	...
	_, err := s.rpc.QRLGetBlockByHash(hash, true)
	s.Require().Nil(err)
	...
	_, err = s.rpc.QRLGetBlockByHash(hash, false)
	s.Require().Nil(err)
}

func (s *QRLRPCTestSuite) TestQRLGetBlockByNumber() {
	// Test with transactions
	number := 3274863
	s.registerResponse(`{}`, func(body []byte) {
		s.methodEqual(body, "qrl_getBlockByNumber")
		s.paramsEqual(body, `["0x31f86f", true]`)
	})

	_, err := s.rpc.QRLGetBlockByNumber(number, true)
	s.Require().Nil(err)
	...
	_, err = s.rpc.QRLGetBlockByNumber(number, false)
	s.Require().Nil(err)
}

func (s *QRLRPCTestSuite) TestQRLCall() {
	s.registerResponse(`"0x11"`, func(body []byte) {
		s.methodEqual(body, "qrl_call")
		s.paramsEqual(body, `[{"from":"0x111","to":"0x222"}, "ttt"]`)
	})

	result, err := s.rpc.QRLCall(T{
		From: "0x111",
		To:   "0x222",
	}, "ttt")
	s.Require().Nil(err)
...
func (s *QRLRPCTestSuite) TestQRLEstimateGas() {
	s.registerResponseError(errors.New("error"))
	result, err := s.rpc.QRLEstimateGas(T{
		From: "0x111",
		To:   "0x222",
	})
	s.Require().NotNil(err)
	...
	s.registerResponse(`"0x5022"`, func(body []byte) {
		s.methodEqual(body, "qrl_estimateGas")
...
func (s *QRLRPCTestSuite) TestQRLGetTransactionReceipt() {
	hash := "0x9c17afa5336d3cfd47e2e795520959b92e627e123e538fd4d5d7ece9025a8dce"
	s.registerResponseError(errors.New("error"))
	receipt, err := s.rpc.QRLGetTransactionReceipt(hash)
	s.Require().NotNil(err)
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "qrl_getTransactionReceipt")
...
func (s *QRLRPCTestSuite) TestGetTransaction() {
	result := `{
...
func (s *QRLRPCTestSuite) TestQRLGetTransactionByHash() {
	s.registerResponse(`{}`, func(body []byte) {
		s.methodEqual(body, "qrl_getTransactionByHash")
		s.paramsEqual(body, `["0x123"]`)
	})

	t, err := s.rpc.QRLGetTransactionByHash("0x123")
	s.Require().Nil(err)
	s.Require().NotNil(t)
}

func (s *QRLRPCTestSuite) TestQRLGetTransactionByBlockHashAndIndex() {
	s.registerResponse(`{}`, func(body []byte) {
		s.methodEqual(body, "qrl_getTransactionByBlockHashAndIndex")
		s.paramsEqual(body, `["0x623", "0x12"]`)
	})

	t, err := s.rpc.QRLGetTransactionByBlockHashAndIndex("0x623", 18)
	s.Require().Nil(err)
	s.Require().NotNil(t)
}

func (s *QRLRPCTestSuite) TestQRLGetTransactionByBlockNumberAndIndex() {
	s.registerResponse(`{}`, func(body []byte) {
		s.methodEqual(body, "qrl_getTransactionByBlockNumberAndIndex")
		s.paramsEqual(body, `["0x1f537da", "0xa"]`)
	})

	t, err := s.rpc.QRLGetTransactionByBlockNumberAndIndex(32847834, 10)
	s.Require().Nil(err)
	s.Require().NotNil(t)
}

func (s *QRLRPCTestSuite) TestQRLNewFilterWithAddress() {
	...
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "qrl_newFilter")
		s.paramsEqual(body, fmt.Sprintf(`[{"address": ["%s"]}]`, address[0]))
	})

	filterID, err := s.rpc.QRLNewFilter(filterData)
	s.Require().Nil(err)
	s.Require().Equal(result, filterID)
}

func (s *QRLRPCTestSuite) TestQRLNewFilterWithTopics() {
	topics := [][]string{
		{
...
func (s *QRLRPCTestSuite) TestQRLNewFilterWithAddressAndTopics() {
	topics := [][]string{
		{"0xb2b2eeeee341e560da3d439ef5e5309d78a22a66"},
		{"0xb2b2fffff341e560da3d439ef5e5309d78a22a66"},
	}
...
func (s *QRLRPCTestSuite) TestQRLNewBlockFilter() {
	result := "0x6996a3a4788d4f2067108d1f536d4330"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "qrl_newBlockFilter")
	})

	filterID, err := s.rpc.QRLNewBlockFilter()
	s.Require().Nil(err)
	s.Require().Equal(result, filterID)
}

func (s *QRLRPCTestSuite) TestQRLNewPendingTransactionFilter() {
	result := "0x153"
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "qrl_newPendingTransactionFilter")
	})

	filterID, err := s.rpc.QRLNewPendingTransactionFilter()
	s.Require().Nil(err)
	s.Require().Equal(result, filterID)
}

func (s *QRLRPCTestSuite) TestQRLGetFilterChanges() {
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "qrl_getFilterChanges")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, filterID))
	})

	logs, err := s.rpc.QRLGetFilterChanges(filterID)
	s.Require().Nil(err)
	s.Require().Equal([]Log{
		{
			Address:     "0xaca0cc3a6bf9552f2866ccc67801d4e6aa6a70f2",
...
func (s *QRLRPCTestSuite) TestQRLGetFilterLogs() {
	filterID := "0x6996a3a4788d4f2067108d1f536d4330"
	result := `[{
		"address": "0xaca0cc3a6bf9552f2866ccc67801d4e6aa6a70f2",
		"blockHash": "0x9d9838090bb7f6194f62acea788688435b79cc44c62dcf1479abd9f2c72a7d5c",
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "qrl_getFilterLogs")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, filterID))
	})

	logs, err := s.rpc.QRLGetFilterLogs(filterID)
	s.Require().Nil(err)
	s.Require().Equal([]Log{
		{
			Address:     "0xaca0cc3a6bf9552f2866ccc67801d4e6aa6a70f2",
...
func (s *QRLRPCTestSuite) TestQRLGetLogs() {
	params := FilterParams{
		FromBlock: "0x1",
		ToBlock:   "0x10",
	...
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "qrl_getLogs")
	...
	logs, err := s.rpc.QRLGetLogs(params)
	s.Require().Nil(err)
	s.Require().Equal([]Log{
		{
			Address:     "0xaca0cc3a6bf9552f2866ccc67801d4e6aa6a70f2",
...
func (s *QRLRPCTestSuite) TestQRLUninstallFilter() {
	filterID := "0x6996a3a4788d4f2067108d1f536d4330"
	result := "true"
	s.registerResponse(result, func(body []byte) {
		s.methodEqual(body, "qrl_uninstallFilter")
		s.paramsEqual(body, fmt.Sprintf(`["%s"]`, filterID))
	})

	uninstall, err := s.rpc.QRLUninstallFilter(filterID)
	s.Require().Nil(err)
	boolRes, _ := strconv.ParseBool(result)
	s.Require().Equal(boolRes, uninstall)
}

func TestQRLRPCTestSuite(t *testing.T) {
	suite.Run(t, new(QRLRPCTestSuite))
}

func TestQRLError(t *testing.T) {
	var err error
	err = QRLError{-32555, "Messg"}
	require.Equal(t, "Error -32555 (Messg)", err.Error())

	err = QRLError{32847, "Kuku"}
	require.Equal(t, "Error 32847 (Kuku)", err.Error())
}

func TestQRL1(t *testing.T) {
	client := NewQRLRPC("")
	require.Equal(t, int64(1000000000000000000), QRL1().Int64())
	require.Equal(t, int64(1000000000000000000), client.QRL1().Int64())
}
