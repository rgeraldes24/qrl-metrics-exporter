...
func collectCommitteeIndicesWithAttestations(ctx context.Context, t *testing.T, service client.Service) map[capella.CommitteeIndex]bool {
	t.Helper()
	...
	indices := make(map[capella.CommitteeIndex]bool)
	for _, attestation := range response.Data {
		committeeBits, err := attestation.CommitteeBits()
		require.NoError(t, err)
		for _, committeeIndex := range committeeBits.BitIndices() {
			indices[capella.CommitteeIndex(committeeIndex)] = true
		}
	}
	return indices
}
