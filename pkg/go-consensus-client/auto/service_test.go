...
import (
	...
	"github.com/theQRL/qrl-metrics-exporter/pkg/go-consensus-client/auto"
)
...
func TestService(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "Qrysm",
			address: os.Getenv("QRYSMGRPC_ADDRESS"),
			version: "Qrysm",
		},
		{
			name:    "BadPort",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.err != "" {
				...
				version, err := service.(consensusclient.NodeVersionProvider).NodeVersion(context.Background(), &api.NodeVersionOpts{})
				require.NoError(t, err)
