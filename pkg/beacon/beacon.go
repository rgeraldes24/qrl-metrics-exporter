...
import (
	...
	eapi "github.com/theQRL/qrl-metrics-exporter/pkg/go-consensus-client/api"
...
type Node interface {
	...
	// QRL getters. These are all cached.
	...
	FetchBlockRoot(ctx context.Context, stateID string) (*capella.Root, error)
	...
	FetchBeaconStateRoot(ctx context.Context, stateID string) (capella.Root, error)
	...
	FetchProposerDuties(ctx context.Context, epoch capella.Epoch) ([]*v1.ProposerDuty, error)
	...
	FetchBeaconCommittees(ctx context.Context, state string, epoch *capella.Epoch) ([]*v1.BeaconCommittee, error)
	// FetchAttestationData fetches the attestation data for the given slot and committee index.
	FetchAttestationData(ctx context.Context, slot capella.Slot, committeeIndex capella.CommitteeIndex) (*capella.AttestationData, error)
	...
	OnVoluntaryExit(ctx context.Context, handler func(ctx context.Context, ev *capella.SignedVoluntaryExit) error)
	// OnContributionAndProof is called when a contribution and proof is received.
	OnContributionAndProof(ctx context.Context, handler func(ctx context.Context, ev *capella.SignedContributionAndProof) error)
...
// Node represents a QRL beacon node. It computes values based on the spec.
type node struct {
	// Helpers
...
func (n *node) Service() consensusclient.Service {
	return n.client
}
	...
	n.wallclock.OnSlotChanged(func(slot ethwallclock.Slot) {
		...
		if err != nil {
			if strings.Contains(err.Error(), "404") {
				n.publishEmptySlot(ctx, capella.Slot(slot.Number()))
			}
...
func (n *node) fetchIsHealthy(ctx context.Context) error {
	...
	if !isProvider {
		return errors.New("client does not implement consensusclient.NodeSyncingProvider")
	}
...
func (n *node) getBlock(ctx context.Context, blockID string) (*spec.VersionedSignedBeaconBlock, error) {
	provider, isProvider := n.client.(consensusclient.SignedBeaconBlockProvider)
	if !isProvider {
		return nil, errors.New("client does not implement consensusclient.SignedBeaconBlockProvider")
	}
...
func (n *node) getBlockRoot(ctx context.Context, blockID string) (*capella.Root, error) {
	provider, isProvider := n.client.(consensusclient.BeaconBlockRootProvider)
	if !isProvider {
		return nil, errors.New("client does not implement consensusclient.SignedBeaconBlockProvider")
	}
