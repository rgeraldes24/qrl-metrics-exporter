...
type SpecMetrics struct {
	...
	SecondsPerExecutionBlock       prometheus.Gauge
	...
	ExecutionFollowDistance        prometheus.Gauge
...
func NewSpecJob(bc Node, log logrus.FieldLogger, namespace string, constLabels map[string]string) *SpecMetrics {
	...
	s := &SpecMetrics{
		...
		SecondsPerExecutionBlock: prometheus.NewGauge(
			prometheus.GaugeOpts{
				Namespace:   namespace,
				Name:        "seconds_per_execution_block",
				Help:        "The number of seconds per Execution block.",
				ConstLabels: constLabels,
			},
		),
		...
		ExecutionFollowDistance: prometheus.NewGauge(
			prometheus.GaugeOpts{
				Namespace:   namespace,
				Name:        "execution_follow_distance",
				Help:        "The number of blocks to follow behind the head of the execution chain.",
				ConstLabels: constLabels,
			},
		),
	...
	prometheus.MustRegister(s.SecondsPerExecutionBlock)
	...
	prometheus.MustRegister(s.ExecutionFollowDistance)
...
func (s *SpecMetrics) observeSpec(ctx context.Context, spec *state.Spec) error {
	...
	s.SecondsPerExecutionBlock.Set(float64(spec.SecondsPerExecutionBlock.AsDuration().Seconds()))
	...
	s.ExecutionFollowDistance.Set(float64(spec.ExecutionFollowDistance))
