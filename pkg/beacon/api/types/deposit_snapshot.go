...
type DepositSnapshot struct {
	Finalized            []capella.Root `json:"finalized"`
	DepositRoot          capella.Root   `json:"deposit_root"`
	DepositCount         uint64         `json:"deposit_count"`
	ExecutionBlockHash   capella.Root   `json:"execution_block_hash"`
...
func (d *DepositSnapshot) UnmarshalJSON(input []byte) error {
	...
	d.Finalized = make([]capella.Root, len(depositSnapshotJSON.Finalized))
