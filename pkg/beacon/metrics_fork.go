...
func (f *ForkMetrics) calculateCurrent(context.Context) error {
	...
	for _, fork := range spec.ForkEpochs {
		...
		if fork.Active(capella.Epoch(capella.Slot(slot.Number()) / slotsPerEpoch)) {
	...
	current, err := spec.ForkEpochs.CurrentFork(capella.Epoch(capella.Slot(slot.Number()) / slotsPerEpoch))
	if err != nil {
