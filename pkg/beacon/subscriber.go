...
func (n *node) OnVoluntaryExit(ctx context.Context, handler func(ctx context.Context, event *capella.SignedVoluntaryExit) error) {
	n.broker.On(topicVoluntaryExit, func(event *capella.SignedVoluntaryExit) {
		n.handleSubscriberError(handler(ctx, event), topicVoluntaryExit)
	})
}

func (n *node) OnContributionAndProof(ctx context.Context, handler func(ctx context.Context, event *capella.SignedContributionAndProof) error) {
	n.broker.On(topicContributionAndProof, func(event *capella.SignedContributionAndProof) {
